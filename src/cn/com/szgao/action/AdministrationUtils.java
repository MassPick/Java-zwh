...
public class AdministrationUtils {
	...
	public String[] enterp(String doString) {
		...
		Map<Integer, String> resultMap = subCompanyDo(oneString, twoString,
				doString);// 提取省市县区
	...
	public String[] doProvinceCityCountry(Map<Integer, String> resultMap) {
		...
		if (null != resultMap) {
			...
			if ("北京市".equals(province_result) || "天津市".equals(province_result)
					|| "重庆市".equals(province_result)
			...
			if (null == city_result && null != country_result) {
				...
				if (null != province_result) {// 有省
					Map<Integer, String> map = utils
							.listCityIdByProvinceName(province_result);
					if (null != map && map.size() != 0) {
						Map<Integer, String> map2 = utils
								.listCountryCityIdBycountryName(country_result);  // 得到查询县名对应的市
																				  // 可能有多行，如长安区就在不同的市存在
						for (Map.Entry<Integer, String> entry : map.entrySet()) { // 得到某省下的市ID
							for (Map.Entry<Integer, String> entry2 : map2
									.entrySet()) {
						...
						// 获得地级市ID
					int city_id = DataUtils.xianJiShiXianQuMap
							.get(country_result);

					if (city_id == 400 || city_id == 401 || city_id == 402
							|| city_id == 403) {
						String province_name = utils
								.listProvinceNameByProvinceId(city_id);
						...
						Map<String, Integer> map = utils
								.listCityProvinceIdByCityId(city_id);
						if (null != map && map.size() != 0) {
							...
							if (null == province_result) {
								...
								// 通过省ID查询省名称
								String province_name = utils
										.listProvinceNameByProvinceId(province_id);
								if (null != province_name) {
									province_result = province_name;
								}
							}
	 ...
	 * @since 2015-6-12
	 */
	public Map<Integer, String> subCompanyDo(String oneString,
			String twoString, String doString) {
			...
			for (int i = 0; i < xianList.size(); i++) {
				...
				if (twoString.contains(tempXian)) {
					...
					if (temp.length() > 0)// 避免 云县人民法院
					{
						...
						if (null != DataUtils.xianJiShiXianMap.get(sub
								+ tempXian)) {
							resultMap.put(
									3,
									DataUtils.xianJiShiXianMap.get(sub
											+ tempXian)); // 县、县级市、旗
							break;
						}
					}
				...
				if (null == resultMap.get(3)) {
					for (int i = 0; i < DataUtils.quList.size(); i++) {
						...
						if (twoString.contains(tempArea)) {
							...
							if (temp.length() > 0)// 避免 云县人民法院
							{
								String sub = twoString.substring(index - 1,
										index);// 云县，再往前推一个字
								if (null != DataUtils.xianJiShiXianMap.get(sub
										+ tempArea)) {
									resultMap.put(
		...
		if (null == resultMap.get(1) && null == resultMap.get(2)
				&& null == resultMap.get(3)) {
			// 在第1截里面找【省】
			for (int i = 0; i < shengList.size(); i++) {
				String tempSheng = shengList.get(i);
				...
				if (null == resultMap.get(3)) {
					for (int i = 0; i < xianList.size(); i++) {
						...
						if (oneString.contains(tempXian)) {
							...
							if (temp.length() > 0)// 避免 云县人民法院
							{
								String sub = oneString.substring(index - 1,
										index);// 云县，再往前推一个字
								if (null != DataUtils.xianJiShiXianMap.get(sub
										+ tempXian)) {
									resultMap.put(
							...
							resultMap.put(3,
									DataUtils.xianJiShiXianMap.get(tempXian)); // 县、县级市、旗
							break;
						}
					}
				}
		...
		if (null != resultMap && 3 != resultMap.size()) {
			if (null == resultMap.get(1)) {
				...
				for (int i = 0; i < shengList.size(); i++) {
					...
					{
						if (doString
								.contains(DataUtils.shengMap.get(tempSheng))) {
							resultMap.put(1, DataUtils.shengMap.get(tempSheng));// 省
							break;
						}
			...
			if (null == resultMap.get(2)) {
				...
				if (!"北京市".equals(resultMap.get(1))
						|| !"重庆市".equals(resultMap.get(1))
						|| !"天津市".equals(resultMap.get(1))
						|| !"上海市".equals(resultMap.get(1))) {
					...
					for (int i = 0; i < shiList.size(); i++) {
						...
						if (doString.contains(tempShi + "路"))// 避免类似 "天津路" 的情况
						{
							// 使用类似 "天津市"去判断
							if (doString
									.contains(DataUtils.shiMap.get(tempShi))) {
								resultMap.put(2, DataUtils.shiMap.get(tempShi)); // 地级市
								break;
							}
					...
					if (null == resultMap.get(3)) {
						for (int i = 0; i < DataUtils.quList.size(); i++) {
							...
							if (doString.contains(tempArea)) {
								...
								if (temp.length() > 0)// 避免 云县人民法院
								{
									String sub = doString.substring(index - 1,
											index);// 云县，再往前推一个字
									if (null != DataUtils.xianJiShiXianMap
											.get(sub + tempArea)) {
										resultMap.put(3,
												DataUtils.xianJiShiXianMap
														.get(sub + tempArea)); // 县、县级市、旗
										break;
									}
								}
								resultMap.put(3, tempArea);
	...
	public String[] court(String doString) {
		...
		quList.addAll(DataUtils.quMap.keySet());

		// 遍历省，在第全截里面找【省】
		for (int i = 0; i < shengList.size(); i++) {
			if (doString.contains(shengList.get(i))) {
				resultMap.put(1, DataUtils.shengMap.get(shengList.get(i)));// 省
				// 去掉省
		...
		if (!StringUtils.isNull((resultMap.get(1)))) {// 有省时
			 map = utils
					.listCountryIdByProvinceName((resultMap.get(1)));
			 if (null != map && map.size() != 0) {
		...
		for (int i = 0; i < xianList.size(); i++) {
			...
			if (doString.contains(tempXian)) {
				...
				if (temp.length() > 0)// 避免 云县人民法院
				{
					...
					if (null != DataUtils.xianJiShiXianMap.get(sub + tempXian)) {
						resultMap.put(3,
								DataUtils.xianJiShiXianMap.get(sub + tempXian)); // 县、县级市、旗
						break;
					}
				}
				resultMap.put(3, DataUtils.xianJiShiXianMap.get(tempXian)); // 县、县级市、旗
